...
export function HomeStateSearchClient({
  ...
  return (
    <div className="space-y-4 rounded-2xl border border-slate-200 dark:border-slate-800 bg-white dark:bg-slate-900/70 p-4 sm:p-5">
      <div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
        ...
        <div className="flex flex-col gap-2 sm:w-64">
          ...
          <select
            className="min-h-[44px] rounded-xl border border-slate-300 dark:border-slate-700 bg-slate-50 dark:bg-slate-900 px-3 text-sm text-slate-900 dark:text-slate-100"
      ...
      {isDelhi ? (
        <div className="space-y-3">
          ...
          {delhiReady ? (
            ...
            <div className="rounded-xl border border-amber-500/40 bg-amber-500/5 p-3 text-xs text-amber-700 dark:text-amber-200">
              Delhi data is still being set up and will be available here soon.
              {delhiStatusMessage && (
                <div className="mt-2 text-[11px] text-amber-800/90 dark:text-amber-200/90">{delhiStatusMessage}</div>
              )}
            </div>
          )}
        </div>
          ...
          {isLive ? (
            ...
            <>
              ...
              <div className="space-y-2 sm:flex sm:items-center sm:gap-3 sm:space-y-0">
                <input
                  type="text"
                  className="min-h-[44px] flex-1 rounded-xl border border-slate-300 dark:border-slate-700 bg-slate-50 dark:bg-slate-900 px-3 text-sm text-slate-900 dark:text-slate-100 placeholder:text-slate-400 dark:placeholder:text-slate-500"
              ...
              {waitlistState.error && (
                <p className="text-xs text-red-600 dark:text-red-400">{waitlistState.error}</p>
              )}
              {waitlistState.message && (
                <p className="text-xs text-emerald-700 dark:text-emerald-300">{waitlistState.message}</p>
              )}
            </>
          )}
        </div>
